...
function parseProperties(src, parse, ignoreUndefined) {
  ...
  Object.keys(parse).forEach((parseName) => {
    ...
    if (typeof fn === 'function') {
      const result = fn(_.get(src, parseName));
      if (isNotUndefined(result)) {
        _.set(src, parseName, result); // eslint-disable-line no-param-reassign
      } else if (ignoreUndefined) {
        _.unset(src, parseName);
      }
    }
  });
  return src;
...
function transform(params) {
  ...
  return parseProperties(transformResult, params.parse, params.ignoreUndefined);
}
